Public Class insCheckProc
    ...
    ' 項目入力値チェック処理
    '====================
    Public Sub CheckData(ByVal inp As insDisplayProc)
        ...
        ' 社員番号
        If String.IsNullOrEmpty(inp.inNo) Then
            ' 入力値が空の場合エラー
            Console.WriteLine("Noが入力されていないため、登録できません。")
            Me.ChkResult = "ERR"
            Exit Sub
        ElseIf Not IsNumeric(inp.inNo) Then
            ' 数値以外の場合エラー
        ...
        ' 社員番号重複チェック
        Dim nocnt As Integer      ' 社員番号件数
        tbl.SelectTable(inp.inNo, nocnt)
        ' 社員番号が既にテーブルに存在した場合、エラー（処理を抜ける）
        If nocnt <> 0 Then
            Me.ChkResult = "ERR"
            Exit Sub
        End If
